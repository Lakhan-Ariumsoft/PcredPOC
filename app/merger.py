...
New extraction format per field:
  { "current": {value, confidence, evidence, page},
    "previous": {value, confidence, evidence, page} }

Each doc has current_fy and previous_fy.
The merger maps: doc.current → doc.current_fy, doc.previous → doc.previous_fy
"""
...
def merge_documents(
    ...
    Each doc_result must have:
      doc_id, filename, current_fy, previous_fy (or primary_fy/secondary_fy),
      status, extraction (or None)
    """
    # Initialise scaffold
    cma_data: dict = {
    ...
    for doc in doc_results:
        ...
        # Support both old primary_fy and new current_fy keys
